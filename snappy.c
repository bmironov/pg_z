...
pg_snappy(PG_FUNCTION_ARGS)
{
	 ...
	 */
	estimated_chunks = (in_size + SNAPPY_CHUNK_SIZE - 1) / SNAPPY_CHUNK_SIZE;
	allocated_size =
	...
	out_buf[out_offset++] = 0x06; // Length: 3 bytes (as little-endian)
	...
	while (src_offset < in_size) {
		...
		total_chunk_len = max_comp_len + 4; // Payload size + 4 bytes of CRC32C

		out_buf[chunk_header_pos++] = chunk_type;
		out_buf[chunk_header_pos++] = (uint8_t)(total_chunk_len & 0xff);
		out_buf[chunk_header_pos++] = (uint8_t)((total_chunk_len >> 8) & 0xff);
...
pg_unsnappy(PG_FUNCTION_ARGS)
{
	...
	while (src_offset < in_size) {
		if (src_offset + 4 > in_size) {
			pg_hybrid_free(out_buf);
			PG_FREE_IF_COPY(in_varlena, 0);
			elog(ERROR, "Snappy decompression failed: malformed chunk header");
		}
		...
		if (src_offset + chunk_len > in_size) {
			...
			PG_FREE_IF_COPY(in_varlena, 0);
			elog(ERROR,
				 "Snappy decompression failed: chunk length out of bounds");
		}
		...
		if ((chunk_type >= 0x02 && chunk_type <= 0x7f) || chunk_type == 0xfe) {
			src_offset += chunk_len;
			continue;
		}
		...
		if (chunk_type == CHUNK_COMPRESSED_DATA ||
			...
			if (chunk_len < 4) {
				pg_hybrid_free(out_buf);
				PG_FREE_IF_COPY(in_varlena, 0);
				elog(ERROR, "Snappy decompression failed: empty data chunk");
			}
			...
			if (new_allocated_size > allocated_size) {
				...
				if (tmp_buf == NULL) {
					...
					pg_hybrid_free(out_buf);
					elog(ERROR,
						 "out of memory during Snappy decompression buffer "
			...
			PG_FREE_IF_COPY(in_varlena, 0);
			elog(ERROR,
				 "Snappy decompression failed: unsupported chunk type 0x%02x",
				 chunk_type);
		}
	}
